...
class VirtualSGM4(TCP.Server):  
    ...
    def __init__(  
        ...
        self.logger.info("Initializing VirtualSGM4 object")  
    ...
    def init_scan(  
        ...
        *args,  
        ...
        self.source_file = None  # No source file, data will be randomly generated  
        ...
        self.positions = list(  
            zip(*[c.ravel() for c in np.meshgrid(*self.coords.values())])  
        )  

    def init_scan_from_file(  
        self,  
        filename: str | Path,  
        scan_name: str = None,  
    ) -> None:  
        ...
        self.logger.debug(f"Initialized scan from file {filename}")  

    def nearest_position_on_grid(self, position: Sequence[float]) -> Tuple[float]:  
    ...
    def is_close(self, a: Tuple[float], b: Tuple[float], tol: float = 1e-5) -> bool:  
        ...
        if self.source_file is not None:  
            with self.source_file as f:  
                ...
                if found_index is not None:  
                    ...
                    self.logger.debug(  
                        "Reading data from file took {:.3f} seconds".format(  
                            time.time() - t0  
                        )  
                    )  
                else:  
                    self.logger.error(f"Position {pos} not found in poslist.")  
                    raise ValueError(f"Position {pos} is not in the list of positions.")  
        else:  
            data = np.random.rand(*self.signal_shape)  
            self.logger.debug(  
                "Generating fake data took {:.3f} seconds".format(time.time() - t0)  
            )  
    ...
    def create_file(
        ...
        self.file.create_dataset(
            name="Entry/Data/ScanDetails/SlowAxis_length", data=self.lengths, dtype="i4"
        )
        self.file.create_dataset(
            name="Entry/Data/ScanDetails/SlowAxis_start", data=self.starts, dtype="f4"
        )
        self.file.create_dataset(
            name="Entry/Data/ScanDetails/SlowAxis_step", data=self.steps, dtype="f4"
        )
        self.file.create_dataset(
            name="Entry/Data/ScanDetails/SlowAxis_names", data=self.dims, dtype="S10"
        )
        self.file.create_dataset(
            name="Entry/Data/ScanDetails/Dimensions", data=self.ndim, dtype="i4"
        )
    ...
    def position_is_allowed(self, axis: int, target: float) -> bool:
        ...
        return (
            self.limits[axis * self.ndim] <= target <= self.limits[axis * self.ndim + 1]
        )
    ...
    def acquire_data(self, changed: Sequence[bool]) -> float:
        ...
        self.logger.info(
            f"Acquired data at position {pos} | output queue size: {self.output_queue.qsize()}"
        )
        return pos, data
    ...
    def write_data(self, position: tuple[float], data: np.ndarray) -> None:
        ...
        pos_ds.flush()
        self.logger.debug(
            "writing data to file took {:.3f} seconds".format(time.time() - t0)
        )
    ...
    async def scan_loop(self) -> None:
        ...
        while True:
            if self.input_queue.empty():
                ...
                # manhattan distance:
                distance = sum(
                    [abs(next_pos[i] - self.current_pos[i]) for i in range(self.ndim)]
                )
                travel_time = distance / self.MOTOR_SPEED
                self.logger.info(
                    f"Moving to {next_pos} | {travel_time:.2f} seconds | {distance:.2f} um | input queue size: {self.input_queue.qsize()}"
                )
            ...
            if self.output_queue.qsize() < 100:
                ...
                self.logger.warning(
                    f"Queue is full, not acquiring data. Queue size: {self.output_queue.qsize()}"
    ...
    def ADD_POINT(self, *args) -> str:
        assert (
            len(args) == self.ndim
        ), f"expected {self.ndim} arguments, got {len(args)}"
    ...
    def LIMITS(self) -> str:
        ...
        limits = " ".join(
            [f"{x},{y}" for x, y in zip(self.limits[::2], self.limits[1::2])]
        )
    ...
    def MEASURE(self) -> str:
        ...
        data_str = " ".join(
            [str(np.round(v, 4).astype(np.float32)) for v in data.ravel()]
        )
...
class SGM4FileManager:
    ...
    def get_last_n_spectra(self, n) -> np.ndarray:
    ...
    def get_reduced_data(
        self, index: int | slice, pipe: Sequence[str | Callable] = ["sum"]
    ) -> np.ndarray:
    ...
    def reduce(self, func: Callable | str) -> np.ndarray | None:
        ...
        try:
            with h5py.File(self.filename, "r", swmr=self.swmr) as f:
                ...
                for line in pos_array:
                    assert (
                        len(line) == self.ndim
                    ), "length of position does not match dimensionality"
                    corr_line = []
            ...
            Warning(
                "File does not contain positions. Probably loading old data. Using axes instead."
            )
    ...
    def limits(self) -> list[tuple[float]]:
        ...
        with h5py.File(self.filename, "r", swmr=self.swmr) as f:
            ...
            assert (
                len(lengths) == len(starts) == len(steps) == self.ndim
            ), "lengths of limits and dimensionality do not match"
            limits = [
                (start, start + step * (length - 1))
                for start, step, length in zip(starts, steps, lengths)
            ]
    ...
    def stops(self) -> list[float]:
        return [
            start + step * (length - 1)
            for start, step, length in zip(self.starts, self.steps, self.lengths)
        ]
    ...
    def axes(self) -> list[np.ndarray]:
        ...
        with h5py.File(self.filename, "r", swmr=self.swmr) as f:
            ...
            steps = f["Entry/Data/ScanDetails/SlowAxis_step"][()]
            axes = [
                np.linspace(start, start + step * (length - 1), length)
                for start, step, length in zip(starts, steps, lengths)
            ]
            return axes
